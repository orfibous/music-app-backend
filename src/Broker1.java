...
class Broker1 extends Node{
    ...
    Broker1(String ip) throws IOException {
        super(ip);
    }

    Broker1(String ip, int port) throws IOException {
        super(ip, port);
    }
    ...
    void acceptPublisherConnections() throws IOException {
        Node p = new Node(Globals.broker_1_ip, Globals.publisher_accept_port1);
        int i = 0;
        while (i < 1) {
            Node n = new Node(p.ip, Globals.publisher_accept_port1);
            n.init();
            nodes.put("p"+i, n);

            if (n.s.isConnected()) {
                Globals.publisher_accept_port1++;
                i++;
            }
        }
    }
    ...
    void acceptConsumerConnections() throws IOException, ClassNotFoundException {
        Node c = new Node(Globals.broker_1_ip, Globals.consumer_accept_port1);
        int i = 0;
        while (i < 1) {
